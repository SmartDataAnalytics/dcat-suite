...
//    protected Path propertyFolder;
    ...
    public static Path uriToPath(Node node) {
...
//            Path symLinkTgtRelFile = idxFullPath.relativize(symLinkTgtAbsFile);
            ...
            try {
                ...
                CatalogResolverFilesystem.allocateSymbolicLink(symLinkTgtAbsFile, idxFullPath, prefix, suffix);
            } catch (Exception e) {
...
//            Path symLinkTgtFile = tgtRelPath.resolve(tgtFileName);
            ...
            try {
                ...
                if (isSymlink) {
                    ...
                    CatalogResolverFilesystem.deleteEmptyDirectory(basePath, symLinkSrcFile.getParent());
                }
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
...
//        Path symLinkTgtFile = relPath.resolve(fileName);
        ...
        try {
            symLinkTgtPaths = Files.exists(symLinkSrcPath)
                    ? CatalogResolverFilesystem.readSymbolicLinks(symLinkSrcPath, prefix, suffix)
...
//            Path tgtRelPath;
//            try {
                Path absTgt = CatalogResolverFilesystem.resolveSymLink(srcToTgt.getKey(), srcToTgt.getValue());
